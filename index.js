...
app.post('/signup', (req, res, next) => {
  const message = requestvalidator.Validation(req.body, validateRegistration)
  if (message) {
  ...
  if (req.body.password !== req.body.confirmPassword) {
    ...
    const hash = crypto.createHash('md5', config.secret).update(req.body.password).digest('hex') // hashing password
...
app.post('/login', (req, res, next) => {
  const message = requestvalidator.Validation(req.body, validateLogin)
  if (message) {
    return next(new RequestError(400, message))
  }
